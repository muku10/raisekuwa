...
<main class="mx-auto max-w-4xl px-4 py-16 lg:px-8">
	<header class="mb-10">
		<p class="text-sm font-bold uppercase tracking-wider text-crimson"><?php esc_html_e( 'Search results', 'nepalkochino' ); ?></p>
		<h1 class="mt-2 text-3xl lg:text-4xl font-black text-primary-deep">
			<?php
			/* translators: %s: search query. */
			printf( esc_html__( 'Results for: %s', 'nepalkochino' ), '<span class="text-crimson">' . esc_html( get_search_query() ) . '</span>' );
			?>
		</h1>
	</header>
	...
	<?php else : ?>
		<p class="text-muted-foreground"><?php esc_html_e( 'No matches — try a different keyword.', 'nepalkochino' ); ?></p>
	<?php endif; ?>
</main>
